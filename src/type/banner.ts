...
// POST /api/banners 요청 바디 -> 클라이언트가 서버로 전송하는 외부 입력값
// 이미지는 클라이언트가 Supabase에 직접 업로드 후 URL/경로를 전달
export type CreateBannerRequest = {
  ...
  imagePath: string;        // storage 경로 ({uuid}.jpg) — 롤백 시 삭제용
...
export type CandidateBanner = {
  ...
  subjectType: SubjectType | '';
  regionText: string;
  imageUrl: string;   // object URL (URL.createObjectURL) — 미리보기용
  imageBlob: Blob;    // 업로드용 원본 Blob
  observedAt: string;
  excluded: boolean;
};
